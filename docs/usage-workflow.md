...
From v0.46.0 onward, the Expert Agent Console also includes a Next Action Workbench. It shows one recommended primary button, whether the current research can move to handoff, what changed after an API update, how many open actions and gate blockers remain, and the next handoff step. This is a workflow guide only; it does not provide investment advice.

From v0.47.0 onward, served dashboards include an Evidence Composer in the Next Action Workbench when the primary blocker requires handoff evidence. The composer pre-fills note, reviewer, suggested evidence file path, and evidence summary fields, writes a local markdown evidence stub through `/api/evidence/compose-and-set`, updates `review_action_state.json`, and refreshes the gate result in the page. Static dashboards still copy CLI commands.

From v0.48.0 onward, that composer response also includes a Reviewer Confidence gate and evidence preview. The dashboard shows whether the newly created evidence looks like a draft, needs review, or is handoff-ready, lists the specific checks, and displays the generated markdown excerpt. This is a quality-control layer for handoff evidence only; it does not change research ratings or provide investment advice.
